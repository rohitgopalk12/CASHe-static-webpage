...
var ctrl = function($scope)
{
    ...
    {      
        ...
        else
        {
            ...
            document.getElementById('fintech-quiz-score').innerHTML = "<h1>Score : "+score+"/5</h1>";

            if(score<=1)
            {
                document.getElementById('fintech-quiz-feedback').innerHTML= "Feedback : "+scoreFeedback[0];
            }
            else if(score>1 && score<=2)
            {
                document.getElementById('fintech-quiz-feedback').innerHTML = "Feedback : "+scoreFeedback[1];
            }
            else if(score>2 && score<=3)
            {
                document.getElementById('fintech-quiz-feedback').innerHTML = "Feedback : "+scoreFeedback[2];
            }
            else
            {
                document.getElementById('fintech-quiz-feedback').innerHTML = "Feedback : "+scoreFeedback[3];
            }
        }  
    }
    ...
    $scope.startFintechQuiz = function()
    {
        ...
        //document.getElementsByClassName('progressBar').style.display = 'block';
    ...
    $scope.startCompetitiveAnalysisQuiz = function()
    {
        ...
        $scope.competitiveAnalysisTimeLeft = 300;
        $scope.currentQ = 16;
        progress = 0;
        $scope.status = 
            [false,false,false,false,false,
        ...
        $scope.downloadTimer = setInterval(function()
        {
            if($scope.competitiveAnalysisTimeLeft <= 0)
            {
                clearInterval($scope.downloadTimer);
                $scope.submitCompetitiveAnalysisQuiz();
            }
            
            min= Math.floor($scope.competitiveAnalysisTimeLeft/60);
            sec = $scope.competitiveAnalysisTimeLeft%60;
            if(sec<10)
            {
                sec="0"+sec;
            }
                ...
                $scope.competitiveAnalysisTimeLeft = $scope.competitiveAnalysisTimeLeft - 1;
        }, 1000);
